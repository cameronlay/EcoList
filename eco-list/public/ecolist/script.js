...
function dbUpdate(){
    ...
    if(count !== 0){
        for (i = 0; i <= count; i++) {
            var itemName = $("#" + count + "item").val();
            var itemQty = $('#' + a + "quantity").val();
            var author = firebase.auth().currentUser.uid;

                fbRef.child('list').push({
                    'item': itemName,
                    'itemQty': itemQty,
                    'author': author
                })
...
/*
var itemName = document.getElementById("")
var itemQty = document.getElementById("")
var fbRef = firebase.database().ref()
var dbRefList = fbRef.child('list')
var dbRefListItems = dbRefList.child('listItems');

dbRefListItems.on('child_added', function(snap) {
...
dbRefListItems.on('child_changed', function(snap) {
